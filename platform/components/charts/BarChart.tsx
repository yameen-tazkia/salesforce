...
/** Horizontal bar chart: thin marks, rounded data-end, per-bar hover. */
export default function BarChart({
  ...
  return (
    <div role="img" aria-label={title ?? "Bar chart"}>
      {title && (
        <p className="mb-3 text-sm font-semibold text-navy-950">{title}</p>
      )}
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-baseline justify-between gap-3">
                ...
                <span
                  className={`text-xs font-semibold tabular-nums ${hover === i ? "text-navy-950" : "text-[var(--ink-muted)]"}`}
                >
